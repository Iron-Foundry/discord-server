...
async def test_broadcast_config_round_trip(session_factory) -> None:
    ...
    async with session_factory() as session:
        rows = (
            await session.execute(
                select(Config).where(Config.guild_id == _GUILD, Config.key == "broadcast")
            )
        ).scalars().all()
    assert len(rows) == 1


async def test_action_log_and_broadcast_isolated(session_factory) -> None:
    ...
    assert (await broadcast.get_config(_GUILD)).role_id == 1
    assert await action_log.get_config(_GUILD) is not None

    async with session_factory() as session:
        keys = (
            await session.execute(
                select(Config.key).where(Config.guild_id == _GUILD)
            )
        ).scalars().all()
