...
class PgBroadcastRepository:
    """PostgreSQL persistence for the broadcast service."""
    ...
    async def ensure_indexes(self) -> None:
        ...
        logger.info("PgBroadcastRepository: ready")
    ...
    async def get_config(self, guild_id: int) -> BroadcastConfig | None:
        ...
        async with self._factory() as session:
            ...
            if row is None:
                return None
            try:
                return BroadcastConfig.model_validate({**row, "guild_id": guild_id})
            except Exception as exc:
                logger.error(
                    "PgBroadcastRepository: failed to parse config for guild {}: {}",
                    guild_id,
                    exc,
                )
                return None
